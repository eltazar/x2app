<?php
	...
	if($citta=='Qui') {		
		if($tipo=='distanza') {
					$sql="SELECT DISTINCT(esercente.IDesercente),esercente.Indirizzo_Esercente,esercente.Citta_Esercente,esercente.Insegna_Esercente,Latitudine,Longitudine,(ACOS(SIN(RADIANS('$lat'))*SIN(RADIANS(Latitudine))+COS(RADIANS(Latitudine))*COS(RADIANS('$lat'))*COS(ABS(RADIANS('$long')-RADIANS(Longitudine))))*6371) as Distanza
					...
					AND attivita_esercente.IdTipologia_Esercente='25'
					AND Latitudine <> '0'
					AND Longitudine <> '0'
					ORDER BY Distanza
					LIMIT $from,$to;";
		...
		if($tipo=='nome') {
			...
			if($tipo=='distanza') {
					$sql="SELECT DISTINCT(esercente.IDesercente),esercente.Indirizzo_Esercente,esercente.Citta_Esercente,esercente.Insegna_Esercente,Latitudine,Longitudine,(ACOS(SIN(RADIANS('$lat'))*SIN(RADIANS(Latitudine))+COS(RADIANS(Latitudine))*COS(RADIANS('$lat'))*COS(ABS(RADIANS('$long')-RADIANS(Longitudine))))*6371) as Distanza
					FROM esercente, contratto_esercente, attivita_esercente,t_orari_spettacoli, wrp_province
					WHERE
					esercente.Attivo_Esercente='1'
					AND DATEDIFF(contratto_esercente.Data_scadenza_Contresercente,NOW())>='0'
					AND contratto_esercente.IDcontratto_Contresercente=t_orari_spettacoli.IDcontratto_Contresercente
