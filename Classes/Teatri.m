...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString
	...
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/RicercaTeatro.php?chiave=%@&&lat=%f&long=%f",chiave,mylatitudine,mylongitudine]];
...
- (int)aggiorna {
	indice+=20;
	[NSThread detachNewThreadSelector:@selector(spinTheSpinner) toTarget:self withObject:nil];
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/teatri.php?prov=%@&lat=%f&long=%f&giorno=%@&ordina=%@&from=%d&to=20",provincia,mylatitudine,mylongitudine,giorno,tiporic,indice]];
	NSLog(@"Url: %@", url);
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	if (indexPath.section==0) {
		...
		NSLog(@"L'id del teatro da visualizzare è %d",i);
		[tableView deselectRowAtIndexPath:indexPath animated:YES];
		detail = [[DettaglioEsercenti alloc] initWithNibName:@"DettaglioEsercenti" bundle:[NSBundle mainBundle]];
		[(DettaglioEsercenti*)detail setIdentificativo:i];
		[detail setTitle:@"Teatro"];
			//Facciamo visualizzare la vista con i dettagli
		[self.navigationController pushViewController:detail animated:YES];
			//rilascio controller
			//[detail release];
