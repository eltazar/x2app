...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString
	...
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/RicercaPubseBar.php?chiave=%@&&lat=%f&long=%f",chiave,mylatitudine,mylongitudine]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section==1) {
		...
		if (cell == nil){
			[[NSBundle mainBundle] loadNibNamed:@"CellaPerosonalizzata" owner:self options:NULL];
			cell=cellapersonalizzata;
		}
...
- (void) ricerca:(NSString*)tiporic {
	...
	indice=0;
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/pubsebar.php?prov=%@&lat=%f&long=%f&giorno=%@&ordina=%@&from=%d&to=20",provincia,mylatitudine,mylongitudine,giorno,tiporic,indice]];
	NSLog(@"Url: %@", url);
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	if (indexPath.section==0) {
		...
		NSLog(@"L'id del pub da visualizzare è %d",i);
