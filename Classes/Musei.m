...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString
	...
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/RicercaMuseo.php?chiave=%@&lat=%f&long=%f",chiave,mylatitudine,mylongitudine]];
	NSLog(@"Url: %@", url);
...
- (int)aggiorna {
	indice+=20;
	[NSThread detachNewThreadSelector:@selector(spinTheSpinner) toTarget:self withObject:nil];
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/musei.php?prov=%@&lat=%f&long=%f&giorno=%@&ordina=%@&from=%d&to=20",provincia,mylatitudine,mylongitudine,giorno,tiporic,indice]];
	NSLog(@"Url: %@", url);
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	if (indexPath.section==0) {
		...
		NSLog(@"L'id del museo da visualizzare è %d",i);
