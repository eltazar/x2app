...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString
	...
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/RicercaCinema.php?chiave=%@&&lat=%f&long=%f",chiave,mylatitudine,mylongitudine]];
...
- (int)aggiorna {
	[NSThread detachNewThreadSelector:@selector(spinTheSpinner) toTarget:self withObject:nil];
	indice+=20;
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/cinema.php?prov=%@&lat=%f&long=%f&giorno=%@&ordina=%@&from=%d&to=20",provincia,mylatitudine,mylongitudine,giorno,tiporic,indice]];
	NSLog(@"Url: %@", url);
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	if (indexPath.section==0) {
		...
		NSLog(@"L'id del cinema da visualizzare è %d",i);
