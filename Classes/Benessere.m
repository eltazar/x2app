...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString
	...
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/RicercaBenessere.php?chiave=%@&lat=%f&long=%f",chiave,mylatitudine,mylongitudine]];
...
- (int)aggiorna {
	indice+=20;
	[NSThread detachNewThreadSelector:@selector(spinTheSpinner) toTarget:self withObject:nil];
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/benessere.php?prov=%@&lat=%f&long=%f&giorno=%@&ordina=%@&from=%d&to=20",provincia,mylatitudine,mylongitudine,giorno,tiporic,indice]];
	NSLog(@"Url: %@", url);
