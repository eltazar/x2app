//
//  DettaglioEsercenteGenerico.m
//  PerDueCItyCard
//
//  Created by Giuseppe Lisanti on 06/07/11.
...
@implementation DettaglioEsercenteGenerico

@synthesize rows, identificativo;
@synthesize dict,tableview,mappa,webView,sito;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	int righeprimasezione=2;	
	int righesecondasezione=3;
	switch (section) {
		case 0:
			if ( [ [NSString stringWithFormat:@"%@",[dict objectForKey:@"Indirizzo_Esercente"]] isEqualToString:@"<null>"] ){
				righeprimasezione--;
			}
			if ( [ [NSString stringWithFormat:@"%@",[dict objectForKey:@"Giorno_chiusura_Esercente"]] isEqualToString:@"<null>"] ){
				righeprimasezione--;
			}
			return righeprimasezione;
			break;
...
- (void)viewDidLoad {
	...
	NSURL *url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/DettaglioEsercenteGenerico.php?id=%d",identificativo]];
	//NSLog(@"Url: %@", url);
