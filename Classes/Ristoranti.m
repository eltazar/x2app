...
- (IBAction)mostratiporicerca:(id)sender{
	...
	if ([tiporicerca selectedSegmentIndex]==1) {
		[self ricerca:@"prezzo"];
		tiporic=[[NSString alloc]initWithString:@"prezzo"];
		[tableview reloadData];

		}
  if ([tiporicerca selectedSegmentIndex]==2) {
		[self ricerca:@"nome"];
...
- (void)mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control {
	...
	detail = [[DettaglioRistoPub alloc] initWithNibName:@"DettaglioRistoPub" bundle:[NSBundle mainBundle]];
	[(DettaglioRistoPub *)detail setIdentificativo:[v ide]];
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString
	...
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner/RicercaRistorante.php?chiave=%@&lat=%f&long=%f",chiave,mylatitudine,mylongitudine]];
	NSLog(@"Url: %@", url);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (cell == nil){
		[[NSBundle mainBundle] loadNibNamed:@"CellaRistoPub" owner:self options:NULL];
	...
	if( [ [NSString stringWithFormat:@"%@",[dict objectForKey:@"Fasciaprezzo_Esercente"]] isEqualToString:@"<null>"] ){
		...
		prezzo.text =[NSString stringWithFormat:@"Prezzo medio: %@€",[dict objectForKey:@"Fasciaprezzo_Esercente"]];	
	}
...
- (int)aggiorna {
	indice+=20;
	[NSThread detachNewThreadSelector:@selector(spinTheSpinner) toTarget:self withObject:nil];
	url = [NSURL URLWithString:[NSString stringWithFormat: @"http://www.cartaperdue.it/partner//ristoranti.php?prov=%@&lat=%f&long=%f&giorno=%@&ordina=%@&from=%d&to=20",provincia,mylatitudine,mylongitudine,giorno,tiporic,indice]];
	NSLog(@"Url: %@", url);
...
- (void)viewDidLoad {
    [super viewDidLoad];
	[NSThread detachNewThreadSelector:@selector(spinTheSpinner) toTarget:self withObject:nil];
	self.title=@"Ristoranti";
	tiporic=[[NSString alloc]initWithString:@"distanza"];
	indice=-1;
	NSLog(@"Entrato in Ristoranti, Latitudine:%f Longitudine:%f Provincia: %@",mylatitudine,mylongitudine,provincia);
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	if (indexPath.section==0) {
		...
		NSLog(@"L'id del ristorante da visualizzare è %d",i);
		[tableView deselectRowAtIndexPath:indexPath animated:YES];
		detail = [[DettaglioRistoPub alloc] initWithNibName:@"DettaglioRistoPub" bundle:[NSBundle mainBundle]];
		[(DettaglioRistoPub*)detail setIdentificativo:i];
		[detail setTitle:@"Esercente"];
			//Facciamo visualizzare la vista con i dettagli
		[self.navigationController pushViewController:detail animated:YES];
			//rilascio controller
